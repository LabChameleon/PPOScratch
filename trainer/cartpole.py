...
from policies.cartpole_feed_forward import FeedForwardAgent
...
def CartPoleTrainer(
    ...
    agent_policy = FeedForwardAgent(input_size=4, ortho_init=True).to(device)
    ...
    mp_env = MultProcEnvGym("CartPole-v1", num_envs=episodes_in_parallel)
    initial_obs = mp_env.reset()
    buffer = RolloutBuffer(episodes_in_parallel, iterations_per_optimization, episodes_in_parallel)
    ...
    with wandb.init(project="PPO_CartPole_FeedForward_Agent") as run:
        ...
        for iteration in range(train_iterations):
            ...
            if avg_reward > 300:
                render_episode("CartPole-v1", agent_policy)
